...
// Фильтрация сообщений для текущего пользователя
$filteredMessages = [];
foreach ($messages as $message) {
    if (strpos($message, ' | ') !== false) {
        list($timestamp, $fromTo, $content) = explode(' | ', $message, 3);
        if (strpos($fromTo, ' -> ') !== false) {
            list($from, $to) = explode(' -> ', $fromTo);
            if ($from === $_SESSION['username'] || $to === $_SESSION['username']) {
